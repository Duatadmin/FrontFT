...
const CurrentProgramTab: React.FC = () => {
  const { currentProgram, loading } = useProgramStore();
  
  // Empty state - no active program
  if (!loading.program && !currentProgram) {
    return (
  ...
  return (
    <div className="space-y-6" data-testid="current-program-content">
      {currentProgram && (
        <>
          <ProgramOverviewCard program={currentProgram} />
          <WeeklyPlanGrid program={currentProgram} />
        </>
      )}
    </div>
  );
