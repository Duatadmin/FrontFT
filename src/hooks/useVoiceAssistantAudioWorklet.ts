...
export default function useVoiceAssistant({
  ...
  const setupWebSocket = useCallback(() => {
    ...
    wsRef.current = ws;
  }, [onTranscriptComplete, state.isListening, stopListening, autoSendToChat, setChatInputText, sendChatMessage]);
  ...
  }, [init, setupWebSocket, state.isBusy, state.mode]);
  ...
  }, [state.isListening, state.mode, stopListening]);

  // Stop recording and finalize transcript
  const stopListening = useCallback(() => {
    // Mark stream as ended to prevent audio chunks from being sent
  ...
  }, [cleanupAudio, sendEndOfStreamWithSilence]);
